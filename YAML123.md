...
## 📌 專案目錄與 CRISP-DM 流程架構
1. **[第一階段：商業需求探索與核心目標定義 (Business Understanding)](#1-第一階段商業需求探索與核心目標定義-business-understanding)**
2. **[第二階段：資料特徵探索、多重共線性診斷與專家評估 (Data Understanding)](#2-第二階段資料特徵探索多重共線性診斷與專家評估-data-understanding)**
   - **[類別型特徵與獨熱編碼 (One-Hot Encoding)](#類別型特徵與獨熱編碼分析-one-hot-encoding)**
3. **[第三階段：資料前處理、異常值過濾與特徵工程實作 (Data Preprocessing)](#3-第三階段資料前處理異常值過濾與特徵工程實作-data-preprocessing)**
4. **[第四階段：多演算法特徵篩選機制與迴歸效能敏感度分析 (Feature Selection Analysis)](#4-第四階段多演算法特徵篩選機制與迴歸效能敏感度分析-feature-selection-analysis)**
5. **[第五階段：多模型迴歸建模、超參數配置與預測成效評估對比 (Modeling & Evaluation)](#5-第五階段多模型迴歸建模超參數配置與預測成效評估對比-modeling--evaluation)**
6. **[第六階段：預測流水線序列化部署與實時推論服務驗證 (Deployment)](#6-第六階段預測流水線序列化部署與實時推論服務驗證-deployment)**

---

## 1. 第一階段：商業需求探索與核心目標定義 (Business Understanding)
...
## 2. 第二階段：資料特徵探索、多重共線性診斷與專家評估 (Data Understanding)
...
## 3. 第三階段：資料前處理、異常值過濾與特徵工程實作 (Data Preprocessing)
...
## 4. 第四階段：多演算法特徵篩選機制與迴歸效能敏感度分析 (Feature Selection Analysis)
...
## 5. 第五階段：多模型迴歸建模、超參數配置與預測成效評估對比 (Modeling & Evaluation)
...
## 6. 第六階段：預測流水線序列化部署與實時推論服務驗證 (Deployment)
